...
def check_compiler_flag(conf, flag, lang):
	return conf.check(fragment = c_cflag_check_code, mandatory = 0, execute = 0, define_ret = 0, msg = 'Checking for compiler switch %s' % flag, cflags = conf.env[lang + 'FLAGS'] + [flag], okmsg = 'yes', errmsg = 'no')  
def check_compiler_flags_2(conf, cflags, ldflags, msg):
	Logs.pprint('NORMAL', msg)
	return conf.check(fragment = c_cflag_check_code, mandatory = 0, execute = 0, define_ret = 0, msg = 'Checking if building with these flags works', cflags = cflags, ldflags = ldflags, okmsg = 'yes', errmsg = 'no')
...
def configure(conf):
	...
	if conf.env['CFLAGS'] and conf.env['LINKFLAGS']:
		check_compiler_flags_2(conf, conf.env['CXXFLAGS'], conf.env['LINKFLAGS'], "Need to test compiler flags %s and linker flags %s" % (' '.join(conf.env['CFLAGS']), ' '.join(conf.env['LINKFLAGS'])))
	elif conf.env['CFLAGS']:
		check_compiler_flags_2(conf, conf.env['CXXFLAGS'], '', "Need to test compiler flags %s" % ' '.join(conf.env['CFLAGS']))
	...
	gst_version_str = conf.check_cfg(package = 'gstreamer-1.0 >= 1.6.0', modversion = "gstreamer-1.0", uselib_store = 'GSTREAMER', args = '--cflags --libs', mandatory = 1)
	conf.check_cfg(package = 'gstreamer-audio-1.0 >= 1.6.0', uselib_store = 'GSTREAMER', args = '--cflags --libs', mandatory = 1)
	conf.check_cfg(package = 'gstreamer-net-1.0 >= 1.6.0', uselib_store = 'GSTREAMER', args = '--cflags --libs', mandatory = 1)
	...
	conf.define('PACKAGE', "gstnxplay")
	conf.define('PACKAGE_BUGREPORT', "http://")
...
def build(bld):
	bld(
		...
		includes = ['.'],
		uselib = ['GSTREAMER'],
		target = 'gstasyncnetclockbin',
		defines = ['HAVE_CONFIG_H'],
		source = bld.path.ant_glob('gst/*.c'),
	...
	bld(
		...
		includes = ['.'],
		uselib = ['GSTREAMER'],
		target = 'timesource-server',
		defines = ['HAVE_CONFIG_H'],
		source = bld.path.ant_glob('timesource/*.c')
	)
